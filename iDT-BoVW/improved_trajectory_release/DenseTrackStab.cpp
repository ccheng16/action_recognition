...
int main(int argc, char** argv)
{
	...
	// std::cout << capture.get(CV_CAP_PROP_FRAME_COUNT) << std::endl;
	...
	// Initialize bounding boxes, direct bb_file info into bb_list
...
//	fprintf(stderr, "video size, length: %d, width: %d, height: %d\n", seqInfo.length, seqInfo.width, seqInfo.height);
	...
	// 200 is the threshold value, small threshold gives more matched points, but also slower
	...
	while(true) {
		...
		// Optical flow requires two frames to compute, so no calculation for the first frame
		...
		if(frame_num == start_frame) {
			...
			// human_mask marks in-bound points as 1 and out_bound points as 0
			// Ignore in-bound points when calculating SURF descriptors
		...
		// match surf features
		// SURF features are only computed on the original scale of image
		if(bb_file)
			InitMaskWithBox(human_mask, bb_list[frame_num].BBs);
		...
		// Merge matched from surf and optical flow
		MergeMatch(prev_pts_flow, pts_flow, prev_pts_surf, pts_surf, prev_pts_all, pts_all);

		// Use matched points to calculate transformation matrix H
		// Take the identity matrix if only a few matched points
		...
		// Use H to warp, eliminate camera jitter
		Mat H_inv = H.inv();
